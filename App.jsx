...
:root {
  ...
  --display: "Rubik Mono One", Impact, "Haettenschweiler", sans-serif;
  /* Aeonik Mono is licensed, so it is named first and used by anyone who
     has it; DM Mono is the free stand-in - same geometric grotesque
     build and close proportions. */
  --mono: "Aeonik Mono", "DM Mono", ui-monospace, "SFMono-Regular", Menlo, monospace;
...
.shell {
  max-width: calc(var(--rail) + var(--col-gap) + 860px + var(--gutter) * 2);
  margin: 0 auto;
...
.eyebrow {
  ...
  font-size: 10.5px;
  font-weight: 500;
  /* mono is already wide; 0.2em on top of it reads as gappy */
...
/* Text links, not buttons: filled and outlined chips beside the
   suggested terms made three competing shapes, and the quiet one could
   not carry a hover tint that passed contrast. */
.act {
  position: relative; padding: 0 0 3px;
  /* the same small caps every other control uses */
  font-family: var(--sans); font-size: 11px; font-weight: 600;
  letter-spacing: 0.06em; text-transform: uppercase;
  color: #1F51FF; background: none; border: none; cursor: pointer;
  transition: color var(--t-micro) var(--soft);
}
.act-quiet { color: var(--ink3); }
.act::after {
...
.act:hover, .act-quiet:hover { color: #1F51FF; }
.act:hover::after { left: 0; right: 0; }
...
.browse-foot { margin: 30px 0 0; font-size: 13.5px; color: var(--ink3); }
.browse-propose {
  position: relative; color: var(--ink2); text-decoration: none;
  transition: color var(--t-micro) var(--soft);
}
.browse-propose::after {
  content: ""; position: absolute; bottom: -2px; left: 50%; right: 50%; height: 1px;
  background: currentColor;
  transition: left var(--t-enter) var(--ease), right var(--t-enter) var(--ease);
}
.browse-propose:hover { color: #1F51FF; }
.browse-propose:hover::after { left: 0; right: 0.06em; }
...
/* Rows, not pills. Two columns where there is room, one where there is
   not, so the block keeps its shape however many terms a category
   holds - and each row carries its definition, so you can find a word
   by what it means rather than only by recognising its name. */
/* Multi-column, not grid: a grid fills across the row, so the terms ran
   A, B in the first row and C, D in the second. Columns fill downward,
   so the first column holds the first half of the alphabet and the
   second holds the rest. It also drops to one column on its own when
...
.bar-link.is-open .caret { transform: rotate(180deg); }
.bar-link.is-open { color: var(--ink); background: rgba(255,255,255,0.42); }

.bar-pop {
  /* -9px, not 0: the filters container sits 9px inside the bar's edge
     (14px padding and a 1px border, less its own -6px pull), so a menu
     flush to the container stopped short of the column below it */
  position: absolute; top: calc(100% + 8px); right: -9px; z-index: 40;
  min-width: 190px; padding: 5px;
  background: var(--card); border: 1px solid var(--rule);
  border-radius: calc(var(--radius) + 3px);
  box-shadow: 0 12px 28px -14px rgba(31,44,58,0.22);
}
...
.bar-pop-i:hover { background: var(--sunk); color: var(--ink); }
...
.bterm:hover { background: color-mix(in srgb, var(--field) 8%, transparent); }
.bterm-name { font-size: 14px; font-weight: 600; color: var(--ink); white-space: nowrap; }
.bterm:hover .bterm-name { color: var(--field); }
.bterm-short {
...
.bar {
  ...
  align-items: center;
  height: 64px;
  /* The bar's box already matched the column; the rail's items are
     indented inside it, so the panel read as overhanging. This insets
     the bar by the same lead, so its edges sit on the words below. */
  margin: 0 calc(var(--rail-lead) + var(--rail-marker-gap));
...
.entries {
  max-width: calc(860px - var(--bar-inset) - var(--chip-pad) - var(--optical-ink));
  padding-top: var(--s4);
}
...
/* An anchor, so the browser draws its own underline - the button beside
   it gets none, which is why the two did not match. The centre-out rule
   on hover is the only underline this should have. */
...
.aud {
  ...
  border-radius: var(--radius); padding: 5px 9px 4px; white-space: nowrap; line-height: 1;
  transition: background var(--t-micro) var(--soft), color var(--t-micro) var(--soft);
}
...
.chip-label {
  ...
  text-transform: uppercase; padding: 6px 9px 4px; border-radius: var(--radius);
...
/* the two sit on one row, at opposite ends */
.tlink-row {
  display: flex; align-items: center; justify-content: space-between;
  gap: 20px;
}
.tlink-row .tlink { margin-top: var(--s3); }
/* Everything the copy link has, so the underline sits the same distance
   under the words and the two labels share a baseline. The extra bottom
   padding was pushing its rule away from the text. */
.flag-link {
  color: var(--ink3);
  text-decoration: none;
  padding: 0 4px 3px 0;
}
.flag-link::after { background: currentColor; }
.flag-link svg { width: 13px; height: 13px; flex: 0 0 auto; }
.flag-link:hover { color: #1F51FF; }
...
/* Proposing, in the site rather than out of it.
 *
 * The site is a static build, so there is no server to post to - which is
 * why this used to be an external form. Netlify collects submissions from
 * static forms, so the panel posts to the page's own origin. Where that is
 * not available (a preview file, another host), it falls back to putting a
 * formatted block on the clipboard so the work is not lost. */
function ProposePanel({ seed, onClose }) {
  const editing = seed.kind === "edit";
  const [state, setState] = useState("idle"); // idle | sending | sent | copied
  ...
  const asText = () =>
    [
      editing ? "Edit to an existing term" : "New term proposed",
      "Term: " + form.term,
      seed.slug ? "Slug: " + seed.slug : null,
      "What it means: " + form.meaning,
      "Example: " + form.example,
      "Who says it: " + form.said_by,
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("no form handler");
      setState("sent");
    } catch (err) {
      /* no handler here - hand the work back rather than lose it */
      try {
  ...
  return (
    <div className="propose is-in" role="dialog" aria-modal="true" aria-label="Propose a term">
      <div className="propose-inner">
        ...
        {state === "sent" || state === "copied" ? (
          <div className="propose-done">
            <h2>{state === "sent" ? "Sent" : "Copied"}</h2>
            <p>
              {state === "sent"
                ? "It goes to whoever holds the vocabulary this quarter. Nothing is published until it has been through the quarterly pass."
                : "There is no form handler on this build, so the proposal is on your clipboard \u2014 paste it to whoever holds the vocabulary."}
            </p>
          ...
          <>
            ...
            <label className="pf-field">
              <span>The word</span>
              <input value={form.term} onChange={set("term")} placeholder="Buffer lane" />
            </label>
            <label className="pf-field">
              <span>What it means</span>
              <textarea rows={3} value={form.meaning} onChange={set("meaning")}
            ...
            <div className="pf-row">
              <label className="pf-field">
                ...
                <input value={form.said_by} onChange={set("said_by")} placeholder="Name, role" />
              </label>
              <label className="pf-field">
...
const EditIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    {/* barrel, nib and the ferrule line across it - without that line it
        read as a slanted rectangle */}
    <path
      d="M14.5 5.5l4 4M4 20l4.5-1 10-10a2.12 2.12 0 00-3-3l-10 10L4 20z"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
...
/* Where "out of date?" goes. A form to begin with; swap it for the
   repo's issue template once there is volume to justify one. */
const SUGGEST_URL = "https://forms.gle/REPLACE-ME";
...
export default function App() {
  ...
  /* null when shut; otherwise { kind: "new" | "edit", term, slug } */
  const [proposing, setProposing] = useState(null);
  const [openGroup, setOpenGroup] = useState(null);

  const searched = useMemo(() => VOCABULARY.filter((e) => matches(e, query)), [query]);
  /* A search looks at everything. Filtering by team inside a search was
     the confusing part: a word could exist, be found, and still show
     nothing because you happened to be on another tab. The tabs narrow
     the list; the search overrides them. */
  const visible = useMemo(
    () => (query.trim() ? searched : searched.filter((e) => inAudience(e, filter))),
    [searched, filter, query]
  );

  /* Did the thing they typed actually exist, or did we just find its
     neighbours? "lane" finds four lanes and no Lane. */
  const exactHit = useMemo(() => {
    const n = query.trim().toLowerCase();
    if (!n) return true;
    return VOCABULARY.some((e) =>
      [e.term, ...(e.aka || [])].some((x) => x.toLowerCase() === n)
    );
  }, [query]);
  ...
  useEffect(() => {
    if (!menuOpen) return;
    const shut = (e) => {
      if (!e.target.closest || !e.target.closest(".bar-menu")) setMenuOpen(false);
    };
    const key = (e) => e.key === "Escape" && setMenuOpen(false);
    ...
    <div className={`pf ${lifted ? "is-lifted" : ""}`} ref={rootRef}>
      ...
      <div className={`browse ${browsing ? "is-in" : ""}`} aria-hidden={!browsing}>
        ...
        <div className="browse-bar">
            ...
            return (
              ...
              >
                ...
                <div className="bgroup-body">
                  <div>
                    {/* rows, not pills: they hold a definition, they
                        stay scannable as the vocabulary grows, and they
                        do not reflow unpredictably when a term is long */}
                    <ul className="bterms">
                      {list.map((e) => (
                        <li key={e.slug}>
                          <button
                          ...
                          >
                            <span className="bterm-name">{e.term}</span>
                            <span className="bterm-short">{e.short}</span>
                          </button>
                        </li>
                      ))}
                    </ul>
          ...
          {/* the same quiet offer the panel makes, at the end of the list */}
          <p className="browse-foot">
            Missing a word?{" "}
            <button
              className="browse-propose"
              type="button"
              onClick={() => {
                setBrowsing(false);
                setProposing({ kind: "new" });
              }}
            >
              Propose it
            </button>
          ...
          <div className="bar">
            ...
            <div className="filters" role="group" aria-label="Filter the list, or browse everything">
              ...
              <div className="bar-menu">
                <button
                  className={`bar-link ${menuOpen ? "is-open" : ""}`}
                  type="button"
                  aria-haspopup="menu"
                ...
                <div className="bar-pop" role="menu" hidden={!menuOpen}>
                  <button
                    className="bar-pop-i"
                    type="button"
                    role="menuitem"
      ...
      <div className="shell body">
        ...
        <main className="entries" ref={entriesRef}>
            {!exactHit && visible.length > 0 && (
              <div className="near-note">
                <p className="eyebrow">No entry</p>
                <h2>I don&rsquo;t keep &ldquo;{query.trim()}&rdquo;.</h2>
                <p>These are the closest words I do.</p>
                <div className="near-actions">
                  ...
                  <button
                    className="act"
                    onClick={() => setProposing({ kind: "new", term: query.trim() })}
                  >
                    Propose it
                  </button>
                </div>
          ...
          {groups.length === 0 ? (
            <EmptyState
              onPropose={(word) => setProposing({ kind: "new", term: word })}
              query={query}
              filter={filter}
              onPick={(slug) => {
                setQuery("");
...
function Card({ entry, delay, isOpen, onToggle, register, onNavigate, onPropose }) {
  const ref = useRef(null);
  const inView = useInView(ref);
      ...
      >
        <span className="card-title">
          ...
          <span className="aud-wrap">
            ...
            {/* canon needs no badge; the other two do, and retired stays
                reachable because its slug is a link people have shared */}
            {entry.status && entry.status !== "canon" && (
              <span className={`aud status-${entry.status}`}>{entry.status}</span>
            )}
          </span>
      ...
      <div className="drawer" id={`drawer-${entry.slug}`}>
        <div className="drawer-inner">
          <div className="drawer-pad">
            ...
            {/* A stale definition looks exactly like a good one, so the
                only way to catch it is to let whoever notices say so
                where they noticed. */}
            <button
              className="tlink flag-link"
              type="button"
              onClick={() => onPropose({ kind: "edit", term: entry.term, slug: entry.slug })}
            >
              <EditIcon />
              Out of date?
            </button>
...
function EmptyState({ query, filter, onPick, onReset, onPropose }) {
  ...
  return (
    <div className="empty">
      <p className="eyebrow">No entry</p>
      <h2>I don&rsquo;t keep &ldquo;{query.trim() || filterName}&rdquo;.</h2>
      <p>
        {suggestions.length
          ? "Either it isn't ours, or nobody has written it down yet. Here is what I do keep that comes close."
          : "Either it isn't ours, or nobody has written it down yet. I would rather say so than invent something."}
      </p>
      <div className="empty-actions">
        <div className="empty-near">
          {suggestions.map((e) => (
